...
PGRAPHDiffToken::PGRAPHDiffToken(bool initialize, FILE *log_stream)
    : registers{0}, log_stream_{log_stream} {
  if (initialize) {
    Capture();
  }
}
...
void PGRAPHDiffToken::DumpDiff() const {
  ...
  for (auto addr : modified_registers) {
    ...
    if (log_stream_) {
      fprintf(log_stream_, "0x%08X: 0x%08X => 0x%08X\n", addr, old_vals[offset],
              new_vals[offset]);
    }
  }
}
